...
def main():
    ...
    if verified:
        ...
        out.append(
            ...
            f" official `rust:<tag>` (e.g. `{base}`), and that the only addition is one layer"
            " holding cargo-chef, plus the linker for `-mold` and `-wild`."
            " `official N + M` means the base image has N layers and we add M on top."
        )
        out.append("")
        out.append(
